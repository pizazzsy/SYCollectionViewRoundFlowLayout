//
//  SYCollectionViewRoundFlowLayout.m
...
static NSString *const SYCollectionViewRoundSection = @"com.SYCollectionViewRoundSection";

@interface SYCollectionViewRoundLayoutAttributes  : UICollectionViewLayoutAttributes

@property (nonatomic, assign) UIEdgeInsets borderEdgeInsets;///间距
@property (nonatomic, strong) SYCollectionViewRoundConfigModel *myConfigModel;///底色样式配置属性
@property (nonatomic ,assign) BOOL isTouchAnimationEnable;///是否开启点按动画
@property (nonatomic, strong) SYCollectionViewTouchAnimationConfigModel *myTouchAnimationConfigModel;///点按动画配置

@end

@implementation SYCollectionViewRoundLayoutAttributes

- (void)setIsTouchAnimationEnable:(BOOL)isTouchAnimationEnable {
    _isTouchAnimationEnable = isTouchAnimationEnable;
    
    //根据开启点按动画设置默认动画数值
    _myTouchAnimationConfigModel = isTouchAnimationEnable ? [SYCollectionViewTouchAnimationConfigModel createDefaultModel] : nil;
}

- (void)setMyTouchAnimationConfigModel:(SYCollectionViewTouchAnimationConfigModel *)myTouchAnimationConfigModel {
    _myTouchAnimationConfigModel = [myTouchAnimationConfigModel isKindOfClass:[SYCollectionViewTouchAnimationConfigModel class]] ?
    myTouchAnimationConfigModel : [[SYCollectionViewTouchAnimationConfigModel alloc]init];
}

@end
@interface SYCollectionReusableView : UICollectionReusableView

@property (weak, nonatomic) SYCollectionViewRoundLayoutAttributes *myCacheAttr;

@end

@implementation SYCollectionReusableView

- (void)applyLayoutAttributes:(UICollectionViewLayoutAttributes *)layoutAttributes{
    [super applyLayoutAttributes:layoutAttributes];
    SYCollectionViewRoundLayoutAttributes *attr = (SYCollectionViewRoundLayoutAttributes *)layoutAttributes;
    _myCacheAttr = attr;
...
- (void)toChangeCollectionReusableViewRoundInfoWithLayoutAttributes:(UICollectionViewLayoutAttributes *)layoutAttributes{
    ...
    if (attr.myConfigModel) {
        SYCollectionViewRoundConfigModel *model = attr.myConfigModel;
        UIView *view = self;
...
- (void)touchAnimationEventWithScaleSxPoint:(CGFloat)scaleSxPoint
    ...
    __weak SYCollectionReusableView *mySelf = self;
...
- (void)decorationViewUserDidSelectEvent {
    ...
    if ([collectionView isKindOfClass:[UICollectionView class]]) {
        //is CollectionView
        id <SYCollectionViewDelegateRoundFlowLayout> delegate  = (id <SYCollectionViewDelegateRoundFlowLayout>)collectionView.delegate;
...
@interface SYCollectionViewRoundFlowLayout ()

@property (nonatomic, strong) NSMutableArray<UICollectionViewLayoutAttributes *> *decorationViewAttrs;

@end
@implementation SYCollectionViewRoundFlowLayout
...
- (void)prepareLayout {
    ...
    id <SYCollectionViewDelegateRoundFlowLayout> delegate  = (id <SYCollectionViewDelegateRoundFlowLayout>)self.collectionView.delegate;
    ...
    //1.初始化
    [self registerClass:[SYCollectionReusableView class] forDecorationViewOfKind:SYCollectionViewRoundSection];
    ...
    for (NSInteger section = 0; section < sections; section++) {
        ...
        if (isCalculateHeaderView) {
            ...
            if (headerAttr &&
                ...
                if (!CGRectIsNull(rect)) {
                    if (self.isCalculateTypeOpenIrregularitiesCell) {
                        rect = [SYCollectionViewFlowLayoutUtils calculateIrregularitiesCellByMinTopFrameWithLayout:self section:section numberOfItems:numberOfItems defaultFrame:rect];
                    }
            ...
            if (self.isCalculateTypeOpenIrregularitiesCell) {
                if (!CGRectIsNull(firstFrame)) {
                    firstFrame = [SYCollectionViewFlowLayoutUtils calculateIrregularitiesCellByMinTopFrameWithLayout:self section:section numberOfItems:numberOfItems defaultFrame:firstFrame];
                }
            }
        }
        ...
        if (isCalculateFooterView) {
            ...
            if (footerAttr &&
                ...
                if (!CGRectIsNull(rect)) {
                    if (self.isCalculateTypeOpenIrregularitiesCell) {
                        rect = [SYCollectionViewFlowLayoutUtils calculateIrregularitiesCellByMaxBottomFrameWithLayout:self section:section numberOfItems:numberOfItems defaultFrame:rect];
                    }
            ...
            //不计算footerView的情况
            if (self.isCalculateTypeOpenIrregularitiesCell) {
                if (CGRectIsNull(lastFrame)) {lastFrame = [SYCollectionViewFlowLayoutUtils calculateIrregularitiesCellByMaxBottomFrameWithLayout:self section:section numberOfItems:numberOfItems defaultFrame:lastFrame];
                }
            }
        }
        
        //获取sectionInset
        UIEdgeInsets sectionInset = [SYCollectionViewFlowLayoutUtils evaluatedSectionInsetForItemWithCollectionLayout:self atIndex:section];
        ...
        //2. 定义
        SYCollectionViewRoundLayoutAttributes *attr = [SYCollectionViewRoundLayoutAttributes layoutAttributesForDecorationViewOfKind:SYCollectionViewRoundSection withIndexPath:[NSIndexPath indexPathForRow:0 inSection:section]];
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect{
    ...
    if (self.collectionCellAlignmentType != SYCollectionViewFlowLayoutAlignmentTypeBySystem
